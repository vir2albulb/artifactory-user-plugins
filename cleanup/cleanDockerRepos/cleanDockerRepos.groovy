...
// Modified by vir2albulb on 12/06/18.

import groovy.json.JsonBuilder
import java.util.concurrent.TimeUnit
import org.artifactory.repo.RepoPathFactory

// usage: curl -X POST http://localhost:8081/artifactory/api/plugins/execute/cleanDockerRepos?params=repos=example-repo-local1,example-repo-local2

// dry-run
// usage: curl -X POST http://localhost:8081/artifactory/api/plugins/execute/cleanDockerRepos?params=repos=example-repo-local1,example-repo-local2;dryRun=true

executions {
    cleanDockerRepos() { params ->
        def deleted = []
        def repos = params['repos'] ? params['repos'] as String[] : String[]
