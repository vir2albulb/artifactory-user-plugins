...
class CleanDockerReposTest extends Specification {
    def 'simple clean docker repos plugin test'() {
        setup:
        ...
        when:
        def resp = artifactory.plugins().execute('cleanDockerRepos').
                      withParameter('repos', 'example-docker-local').sync()
